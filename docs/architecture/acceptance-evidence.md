...
| Hosted schema and media exposure | Run `npm run db:apply-hosted` for the explicit `SUPABASE_PROJECT_REF` | The lockfile-pinned CLI applies `supabase/config.toml`; the verifier proves effective `db_schema` excludes application schemas and the public `product-images` bucket has the exact size and MIME allowlist, without exposing the service-role key to config push. |
...
| Hosted exposure | Supabase project API and Storage settings export | Data API is disabled or exposes only an explicit empty schema; `app_private` is absent; `product-images` is public with the reviewed 5 MiB JPEG/PNG/WebP contract. |
