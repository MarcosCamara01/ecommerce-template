...
const ProductSkeleton = (
    { extraClassname, numberProducts }:
        { extraClassname: string, numberProducts: number }
) => {
    const productSkeletons = Array.from({ length: numberProducts }, (_, index) => (
        <div key={index} className={`flex justify-between border border-solid border-border-primary rounded-md overflow-hidden 
