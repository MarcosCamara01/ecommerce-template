    ...
    for (const cartItem of cart.items) {
        try {
            if (cartItem.productId && cartItem.variantId) {
                const matchingProduct = await Product.findById(
                    cartItem.productId,
                );
...
export async function setTotalItems(
    productId: Schema.Types.ObjectId,
    size: string,
    variantId: string,
    totalItems: number,
    ...
    try {
                ...
                items: cart.items
                    .map((item) => {
                        if (
                            ...
                            if (totalItems >= 1) {
                                // Update the item's quantity to totalItems
                                item.quantity = totalItems;
                                return item;
                            } else {
                                // Remove the item if totalItems is less than 1
                                return null;
                            }
                        }
                        return item;
        ...
        console.error("Error in setTotalItems:", error);
    }
}
